...
Plus `sitemap.xml`, `robots.txt`, `search-index.json`, `site.webmanifest`, the
favicon pack and a `404.html`.
...
static/
  ...
  favicons/                the favicon pack, copied to the site root at build
scripts/
  make_symbol_svgs.py      regenerates static/symbols/
  make_joint_svgs.py       regenerates static/joints/
  make_favicons.py         regenerates static/favicons/ (needs Pillow)
...
The favicon pack is a simplified fillet weld symbol — a reference line with the
fillet triangle below it, which is the header logo with the leader and arrowhead
dropped, because at 16 pixels the arrow collapses into noise.
...
That writes `static/favicons/`, which is committed; `generate_site.py` copies it to
the site root at build time. Keeping generation in a separate script is why the build
itself still needs only Jinja2 — Pillow is not a build dependency.

| File | Notes |
|---|---|
| `favicon.ico` | 16 and 32 px, each rendered at its own size rather than downscaled |
| `favicon-16x16.png`, `favicon-32x32.png` | rounded corners, transparent outside |
| `favicon.svg` | modern browsers prefer this; scales to any size |
| `apple-touch-icon.png` | 180 px, full bleed — iOS applies its own rounding |
| `mstile-150x150.png` | 150 px, full bleed for Windows tiles |
| `android-chrome-192x192.png`, `android-chrome-512x512.png` | referenced by the manifest |

`site.webmanifest` is generated by `generate_site.py` from `data/site.json`, so the
name, description and theme colour stay in sync with the rest of the site.
